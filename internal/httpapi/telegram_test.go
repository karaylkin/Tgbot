...
func TestValidateInitDataOK(t *testing.T) {
	...
	initData := buildSignedInitDataWithAlgo(t, token, user, time.Now(), true)
...
func TestValidateInitDataExpired(t *testing.T) {
	...
	initData := buildSignedInitDataWithAlgo(t, token, user, past, true)
...
func TestValidateInitDataLegacy(t *testing.T) {
	token := "123456:ABCDEF"
	user := TelegramUser{ID: 55, Username: "legacy"}
	initData := buildSignedInitDataWithAlgo(t, token, user, time.Now(), false)

	if _, err := ValidateInitData(initData, token); err != nil {
		t.Fatalf("legacy validation should pass: %v", err)
	}
}

func buildSignedInitDataWithAlgo(t *testing.T, token string, user TelegramUser, ts time.Time, webApp bool) string {
	t.Helper()
