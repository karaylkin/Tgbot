...
func ValidateInitData(initData string, botToken string) (TelegramUser, error) {
	user, err := validateInitDataOnce(initData, botToken)
	if err == nil {
		return user, nil
	}

	// Fallback: если в строке есть '+', попробуем интерпретировать их как литеральные плюсы.
	// На некоторых клиентов (или после прокси) '+' могли превратить в пробелы до подписи.
	if strings.Contains(initData, "+") {
		alt := strings.ReplaceAll(initData, "+", "%2B")
		if u2, err2 := validateInitDataOnce(alt, botToken); err2 == nil {
			return u2, nil
		}
	}
...
func validateInitDataOnce(initData string, botToken string) (TelegramUser, error) {
	if initData == "" {
